...
module.exports = class NeuralNetwork {
  constructor ({
    ...
    batchSize = 1,
    activation = 'sigmoid',
    testFn = (outputs, targets) =>
      Matrix.perceptron(outputs).compareMatrix(targets),
    logTests = false,
    trackTrainingSuccess = false
  }) {
    if (layers && layers.some(layer => !(layer instanceof Layer))) {
      throw new Error(
        'NeuralNetwork Constructor - Layers should be passed as Layer objects, did you mean layerSizes?'
      )
    }
    if (layers == null && layerSizes.length < 2) {
      throw new Error(
        'NeuralNetwork Constructor - At least 2 layerSizes are required'
      )
    }
    this.layerSizes = (layers && layers.length) || layerSizes
    this.layers =
      layers ||
      layerSizes.reduce(
        (acc, nodes, index) =>
          index ? [...acc, new Layer(nodes, layerSizes[index - 1])] : acc,
        []
      )
  ...
  static fromJSON (json) {
    ...
    settings.layers.forEach((v, i, a) => {
      a[i] = Layer.fromJSON(v)
    })
    return new NeuralNetwork(settings)
  }

  static fromTemplate (template) {
    if (!(template instanceof NeuralNetwork)) {
      throw new Error(
        'NeuralNetwork FromTemplate - Template must be a NeuralNetwork'
      )
    }
  ...
  static crossover (a, b) {
    if (!(a instanceof NeuralNetwork) || !(b instanceof NeuralNetwork)) {
      throw new Error(
        'NeuralNetwork Crossover - 2 NeuralNetworks must be provided'
      )
    }
    const layers = []
  ...
  train (inputs, targets) {
    ...
    for (const layer of this.layers) {
      ...
      activation = Matrix.dot(layer.weights, activation)
